...
# ==========================================
async def mock_llm_query(agent_id: int, query: str, round_num: int, previous_answers: List[str], role: str = "math_expert") -> str:
    """
    ローカルの Ollama (qwen2.5:3b) を呼び出し、討論用の発言を生成する。
    """
    if role == "auditor":
        # 批判担当（Devil's Advocate）ペルソナ用のプロンプト
        if round_num == 0:
            prompt = (
                f"あなたは数学の厳格な監査役（Agent {agent_id}）です。\n"
                f"他のメンバーの計算ミスや論理の隙を徹底的に監査するのがあなたの仕事ですが、\n"
                f"まずは自分自身で厳密に計算を解いてください。思考プロセスを詳細に記述した上で、\n"
            ...
            prompt = (
                f"あなたは討論の監査役（Agent {agent_id}）です。同調圧力に負けてはいけません。\n"
                f"以下に他のエージェントたちの前ラウンドの発言が提示されています。彼らの論理 of 矛盾、\n"
                f"計算ミス、思い込みによる間違いを**絶対に妥協せず、徹底的に暴いて指摘してください**。\n"
                f"他のエージェントと意見が違っていても、正しい論理を貫き通し、監査レポートのように指摘しなさい。\n\n"
                f"【問題】\n{query}\n\n"
                f"【他のエージェントの前ラウンドの発言】\n{other_responses_formatted}\n\n"
...
# ==========================================
class Agent:
    def __init__(self, agent_id: int, role: str = "math_expert"):
...
class MultiAgentDebateManager:
    def __init__(self, num_agents: int, num_rounds: int):
        ...
        # 3人以上の場合にID 1のエージェントを批判担当にする
        self.agents = [
            Agent(i, role="auditor" if (i == 1 and num_agents >= 3) else "math_expert") 
    ...
    async def run_debate(self, query: str) -> str:
        ...
        tasks = [
            mock_llm_query(agent.agent_id, query, 0, [], agent.role)
            for agent in self.agents
        ]
        responses = await asyncio.gather(*tasks)
        for agent, resp in zip(self.agents, responses):
            agent.history.append(resp)
            logger.info(f"Agent {agent.agent_id} ({agent.role}) (Round 0) -> Extracted: {self.extract_answer(resp)}")
        ...
        for r in range(1, self.num_rounds + 1):
            ...
            for agent in self.agents:
                ...
                task = mock_llm_query(agent.agent_id, query, r, other_responses, agent.role)
            ...
            for agent, resp in zip(self.agents, round_responses):
                agent.history.append(resp)
                logger.info(f"Agent {agent.agent_id} ({agent.role}) (Round {r}) -> Extracted: {self.extract_answer(resp)}")
...
async def main():
    ...
    logger.info("======= 実験開始: 討論あり（Devil's Advocate導入） (エージェント数=3, 批判担当=1, ラウンド数=3) =======")
    manager_debate = MultiAgentDebateManager(num_agents=3, num_rounds=3)
    debate_correct = 0
    ...
    print(f"2. 討論エージェント (K=3, R=3, Devil's Advocate=1):")
