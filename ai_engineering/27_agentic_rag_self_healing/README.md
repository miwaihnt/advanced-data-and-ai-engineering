...
> **【Agentic RAG with Self-Healing（フレームワーク不使用・スクラッチ実装）】**
> 本リポジトリは、既存のRAGフレームワーク（LangChain/LlamaIndex/LangGraph等）に依存せず、Self-Healing（自己修復・再試行）メカニズムを備えたRAGシステムをスクラッチで構築・検証するためのリサーチ実装です。
> プロダクション環境における環境制約（セキュリティ・隔離環境）に対応するための設計思想と堅牢性を重視しています。
...
> ⚠️ **なぜフレームワークを排除してスクラッチで実装するのか？**
> オンプレミスやセキュリティ規制の厳しいエンタープライズ環境（エアギャップ・Gov-Cloud環境等）では、「サードパーティライブラリが使用できない」という制約に直面することがあります。
> ライブラリの内部動作を理解し、スクラッチで実装・制御できる能力は、あらゆるシステム環境への高適応力を担保するために不可欠です。

---

## 📊 設計評価ポイント（Architecture Evaluation Points）

| 項目 | 評価ポイント |
|:---|:---|
| **Ingestion & Vector Store** | コサイン類似度の実装、メタデータ設計の妥当性 |
| **Pydanticスキーマ** | `ValidationError` ハンドリングの実装、型安全性 |
| **Self-Healingループ** | 再試行ロジックの正確性、Fallback処理、無限ループ防止 |
| **動作検証** | 3つのクエリに対する期待動作の確認 |
| **設計判断の言語化** | 「なぜこの設計か」「代替案は何か」のドキュメント化 |

---

## 💬 設計上の技術的考察（Design Questions）

本システムを設計・運用するにあたり、以下の問いに対する考察を行っています。
...
*実装の参照例は `answer/` ディレクトリに格納されています。*
