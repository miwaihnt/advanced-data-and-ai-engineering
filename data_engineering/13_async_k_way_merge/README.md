...
3. **別解（非同期ジェネレータ関数）とのトレードオフ**:
   `yield` を用いた非同期ジェネレータ関数（`async def stream_func()`）でも同様のイテレータは作成できますが、あえて `__aiter__` と `__anext__` 特殊メソッドを持つクラスとして実装することで、Pythonの低レイヤーにおけるイテレータプロトコルおよび `StopAsyncIteration` 例外のライフサイクルの理解度を面接官にアピールすることができます。
